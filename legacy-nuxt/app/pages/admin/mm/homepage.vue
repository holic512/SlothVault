...
const homeData = ref<SystemHomepageDto | null>(null)
const content = ref('')
...
async function apiFetch<T>(url: string, options?: any): Promise<T> {
  ...
  throw new Error(res?.message || t('AdminMM.homepage.messages.loadFailed'))
}

// 获取或自动创建系统首页内容
async function fetchOrCreateHomepage() {
  loading.value = true
  try {
    // 先尝试获取
    const data = await apiFetch<SystemHomepageDto>('/api/admin/mm/systemHomepage', {
      method: 'GET',
    })
    homeData.value = data
    ...
    if (e?.message === 'Not Found' || e?.message?.includes('404')) {
      ...
      try {
        ...
        if (createErr?.message !== 'Unauthorized') {
          ElMessage.error(t('AdminMM.homepage.messages.loadFailed'))
        }
      }
    } else if (e?.message !== 'Unauthorized') {
      ElMessage.error(e?.message || t('AdminMM.homepage.messages.loadFailed'))
    }
  } finally {
    loading.value = false
  }
...
// 保存系统首页内容
async function saveHomepage(silent = false) {
  if (saving.value || !homeData.value) return
  if (!hasUnsavedChanges.value) {
    if (!silent) ElMessage.info(t('AdminMM.homepage.messages.contentRequired'))
    return
  }

  saving.value = true
  try {
    const data = await apiFetch<SystemHomepageDto>(`/api/admin/mm/systemHomepage/${homeData.value.id}`, {
      method: 'PUT',
      body: {
    ...
    if (!silent) ElMessage.success(t('AdminMM.homepage.messages.saveSuccess'))
  } catch (e: any) {
    if (e?.message !== 'Unauthorized') {
      ElMessage.error(e?.message || t('AdminMM.homepage.messages.saveFailed'))
    }
  } finally {
    saving.value = false
  }
  ...
  <div class="homepage-editor-page">
    ...
    <div class="toolbar">
      <div class="toolbar-left">
        <span class="page-title">{{ t('AdminMM.homepage.title') }}</span>
      ...
      <div class="toolbar-right">
        ...
        <el-button type="primary" :loading="saving" @click="saveHomepage(false)">
          {{ t('AdminMM.homepage.actions.save') }}
        </el-button>
      </div>
    </div>
