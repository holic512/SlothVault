...
ALTER TABLE `auth_user`
    ...
    ADD COLUMN `wallet_address` VARCHAR(64) NULL,
...
ALTER TABLE `docs_note_info` ADD COLUMN `author_id` INTEGER NULL;
...
CREATE TABLE `points_transaction` (
    ...
    INDEX `idx_points_transaction_user_created`(`user_id`, `created_at`),
    INDEX `idx_points_transaction_reference`(`type`, `reference_id`),
    PRIMARY KEY (`id`)
...
CREATE TABLE `points_gift_card_batch` (
    ...
    INDEX `idx_gift_card_batch_creator`(`created_by_id`),
    INDEX `idx_gift_card_batch_status_expires`(`status`, `expires_at`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `points_gift_card` (
    ...
    `status` SMALLINT NOT NULL DEFAULT 1,
    `redeemed_by_id` INTEGER NULL,
    `redeemed_at` DATETIME(3) NULL,
    `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    UNIQUE INDEX `uq_gift_card_code_hash`(`code_hash`),
    INDEX `idx_gift_card_batch_status`(`batch_id`, `status`),
    INDEX `idx_gift_card_redeemed_by`(`redeemed_by_id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
ALTER TABLE `solana_compressed_nft`
    ADD COLUMN `note_info_id` INTEGER NULL,
