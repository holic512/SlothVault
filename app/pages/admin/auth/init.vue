...
async function onSubmit() {
  if (!form.username || !form.password) {
    errorText.value = t('AdminInit.status.required')
    return
  }
  loading.value = true
  errorText.value = ''
    ...
    <main class="login-container">
      <div class="sloth-card login-card animate-entry">
        ...
        <header class="login-header">
          <div class="sloth-badge-hash hero-badge">{{ t('AdminInit.hero.badge') }}</div>
          <h1 class="login-title">{{ t('AdminInit.hero.title') }}</h1>
          <p class="login-desc">{{ t('AdminInit.hero.desc') }}</p>
        </header>

        <form @submit.prevent="onSubmit" class="login-form">
          <div class="form-group">
            <label class="form-label" for="username">{{ t('AdminInit.form.username') }}</label>
            <div class="input-wrapper">
              <input
                  ...
                  type="text"
                  class="form-input"
                  :placeholder="t('AdminInit.form.username')"
          ...
          <div class="form-group">
            <label class="form-label" for="password">{{ t('AdminInit.form.password') }}</label>
            <div class="input-wrapper">
              <input
                  id="password"
                  ...
                  type="password"
                  class="form-input"
                  :placeholder="t('AdminInit.form.password')"
          ...
          <div class="form-actions">
            ...
            <button class="sloth-btn sloth-btn-primary full-width" type="submit" :disabled="loading">
              ...
              <span v-else>{{ t('AdminInit.form.submit') }}</span>
            </button>
          </div>
        </form>
      </div>
